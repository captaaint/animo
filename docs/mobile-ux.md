...
| 3 | Focusing any input **auto-zoomed** the page (theme base font is 13px; iOS zooms when a focused control is < 16px). | Global CSS bumps native `input`/`textarea`/`select` to **16px** under `@media (pointer: coarse)` only. | `app/index.html` `<style>` | `d26c04b` |
...
### Inputs (no auto-zoom)
- Never let a focusable text control render below **16px** on touch. The global
  rule in `app/index.html` handles this for all native inputs — don't override
  input `fontSize` to something smaller for mobile.
- Never disable zoom to "fix" auto-zoom (no `user-scalable=no` / `maximum-scale`);
  that breaks accessibility. Raising font-size is the correct fix.
...
- Keep `width=device-width, initial-scale=1`. Don't add `user-scalable=no` or
  `maximum-scale`.
...
- The base `fontSize` is `13px` (`tracker-theme.ts`); the 16px input bump is a
  mobile-only CSS layer, not a theme change, so desktop density is preserved.
